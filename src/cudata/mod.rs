...
/// You really shouldn't 'use cudata::*' because of the conflicting method names with other modules.
    ...
    pub fn next_matrix<'b, 'c>(&'c mut self, rows: usize, cols: usize) -> Option<CuMatrixSliceMut<'b>> where 'a: 'b, 'b: 'c {
        ...
        match len <= self.len {
            true => {
                ...
                self.ptr = unsafe { self.ptr.offset(len as isize) };
                self.len -= len;
                Some(CuMatrixSliceMut { _parent: PhantomData, rows, cols, len, ptr })
            }
            false => None
        }
    }
...
pub fn as_vector_slice<'a>(data: &'a CuPackedData) -> CuVectorSlice<'a> {
    CuVectorSlice {
        _parent: PhantomData,
        len: data.len(),
        ptr: data.as_ptr(),
...
pub fn as_vector_slice_mut<'a>(data: &'a mut CuPackedDataMut) -> CuVectorSliceMut<'a> {
    CuVectorSliceMut {
        _parent: PhantomData,
        len: data.len(),
        ptr: data.as_mut_ptr(),
